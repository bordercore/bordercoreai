...
def get_weather_info(model_type, model_name, command):
    ...
    prompt = f"""
    I will give you a series of statements that describes either the current weather, or the weather forecast. I want to answer a weather question based on those statements and nothing else. In particular, if the question is a general query about the weather, say 'Currently it's 79 degrees with mostly sunny skies'. But instead of that particular weather statement substitute the actual weather conditions based on the statements. The weather question is the following: {command}. Answer that question based on the following series of statements about the weather: {weather_description}.
    """
